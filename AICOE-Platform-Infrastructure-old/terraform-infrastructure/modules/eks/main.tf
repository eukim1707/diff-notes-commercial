...
resource "aws_eks_node_group" "eks-node-group-infrastructure" {
  ...
  node_group_name = "${local.eks_cluster_name}-infrastructure-${count.index+1}"
...
resource "aws_eks_node_group" "eks-node-group-workers" {
  count = length(var.private_subnets)
  cluster_name    = local.eks_cluster_name
  node_group_name = "${local.eks_cluster_name}-workers-${count.index+1}"
  node_role_arn   = aws_iam_role.eks_worker_iam_role.arn
  subnet_ids      = [var.private_subnets[count.index]]
...
resource "aws_eks_node_group" "eks-node-group-gpus" {
  count = length(var.private_subnets)
  cluster_name    = local.eks_cluster_name
  node_group_name = "${local.eks_cluster_name}-gpus-${count.index+1}"
  node_role_arn   = aws_iam_role.eks_worker_iam_role.arn
  subnet_ids      = [var.private_subnets[count.index]]
